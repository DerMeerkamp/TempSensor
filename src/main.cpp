...
const char wifiInitialApPassword[] = "test123";
...
void loop() 
{
  ...
  if (60000 < now - lastReport)
  {
    ...
    Serial.println(t);
    mqttClient.publish((std::string("iot/") + iotWebConf.getThingName() + "/sensor/humidity").c_str(), String(h));
    mqttClient.publish((std::string("iot/") + iotWebConf.getThingName() + "/sensor/temperature").c_str(), String(t));
  }
}
